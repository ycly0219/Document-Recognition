...
- **组装报文区（Request Pane）**：接口发送二级窗口上方的只读展示区，按当前模板固定展示组装好的 putPurchaseOrder 或 putOriginalSalesOrder 报文。
...
- **虚拟收货人（Virtual Consignee）**：拣货单 `putOriginalSalesOrder` 报文头部 `consigneeName` 使用的固定收货人名称，当前固定为 `虚拟收货人`。
...
- **客户/供应商（OSCAR Supplier）**：GE-OSCAR 拣货单预览单据头中供应商相关字段的展示名称；表示该主体可能是客户也可能是供应商。
- **序列号（OSCAR Serial No.）**：GE-OSCAR 拣货单明细中的物料序列号；OCR 返回 `N/A` 或空时表示无可用序列号，预览保留 OCR 原文，下游导出/接收按空值处理。_Avoid_: Serial（该词仅用于 GE-ORACLE 拣货单明细）。
