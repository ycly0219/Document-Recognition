...
- 存在 `结果未生成` 页签时「确认并导出」不可用，需先继续查询原任务；待查询任务仅保存在内存，程序重启后需重新提交
- 按单据类型解析头部字段和明细字段
...
4. 等待处理完成，界面日志会展示上传、OCR 提交、轮询和解析进度。若单个文件 5 分钟无结果，页签会显示 `结果未生成`；点击底部「继续查询原任务」按原任务继续等待，不重新上传，续查成功自动刷新为成功。
